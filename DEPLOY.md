...
- 只接受教育部校園樹木地圖的**官方樹號**:在樹木地圖框選帶入,或貼上官方樹號清單 → 產生標籤 → 列印 / 另存 PDF(不是官方樹號的會被略過並列出)
...
- **匯出(主:.xlsx;次:CSV)**:
  ...
  - 主按鈕「下載 Excel (.xlsx)」:欄位一定分開、樹號/樹高/樹圍/筆數是數字;用 Excel 開啟即可。
  - 次按鈕「改下載 CSV」:給需要純文字格式的系統(UTF-8 含 BOM)。
  - 改用 .xlsx 的原因:Excel 依 Windows「清單分隔符號」拆 CSV,實測整列擠在 A 欄。
