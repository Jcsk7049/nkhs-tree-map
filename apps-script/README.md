...
> **重要:更新 `Code.gs` 後,必須重新貼上並以「新版本」重新部署,否則新功能不會生效。**
...
| 能做什麼 | 送出量測 | 管理學生名單、發/重設通行碼、停用學生、解除單一學生鎖定、管理教師信箱 |
...
- `roster-list`:列出學生,每位含 `locked`(是否因連錯被鎖定)
- `roster-unlock`:解除單一學生鎖定(只清連錯計數,不換通行碼);找不到學生回 `VALIDATION_FAILED`
- `teacher-list`:列出教師信箱(小寫、略過空列)
- `teacher-add`:新增教師信箱(轉小寫、重複回 `duplicate: true`);防呆:格式錯誤被拒、上限 100 位
- `teacher-remove`:移除教師信箱;防呆:不能移除自己、找不到被拒、至少保留一位老師
...
- [ ] 被鎖定的學生(連錯 10 次)能在名單頁被老師「解除鎖定」,之後用原通行碼可再送出
- [ ] 教師信箱可新增與移除,且被移除的老師立即無法登入
