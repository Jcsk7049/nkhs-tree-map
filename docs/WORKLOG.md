...
## 2026-09-26 樹號查找 + QR 只用官方樹號(未 commit)
- 老師地圖、學生選樹頁加「輸入樹號」搜尋框(建議清單 861 個樹號;全形/空白/「樹號」字樣可認;找到就飛過去開說明框,老師地圖若被樹種篩選藏起來會先顯示全部)。`src/treeSearch.js`。
- QR 標籤頁移除自編範圍(前綴/起迄/位數,`generateRange` 與其測試一併刪除);產生前用 `partitionOfficial` 比對官方 861 棵,非官方號碼略過並列出;官方資料沒載入就不產生。
- 「國土測繪中心」只提供空拍底圖,樹號來源是教育部校園樹木地圖(edutreemap.moe.edu.tw),已跟使用者確認就是 43667 這種官方樹號。
- 快取 v22,484 測試全過;瀏覽器驗證:全形搜尋 43667 飛過去並開說明框、99999 提示找不到、375px 不溢出;QR 頁(假登入)只產生官方號碼、A-001/12 被列為略過。

## 2026-09-26 地圖說明框「列印這棵的 QR 標籤」(未 commit)
- 老師地圖說明框新增綠色按鈕:存入樹號(`tree-map-print-one`,與列印清單分開)→ 殼層內切到 QR 標籤分頁(獨立開啟則導向 qrcodes.html)→ 只放這一棵並自動產生,按「列印/另存 PDF」即可。
