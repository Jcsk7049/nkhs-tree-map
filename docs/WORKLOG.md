...
- 離線:Service Worker(`public/sw.js`,目前 `tree-map-v13`,安裝時 `cache:'reload'`);`swCacheList.js` 與 `sw.js` 的清單必須一致(有測試守)。
...
- **未驗證(安裝版 App)**:Google 登入在 iframe 內、真機安裝(Android/iOS)、離線開啟殼層與量測頁,皆尚未在真機驗證。
